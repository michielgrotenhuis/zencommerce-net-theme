...
function yoursite_ajax_toggle_currency_status() {
    ...
    if (!$currency_id || !in_array($status, ['active', 'inactive'], true)) {
        wp_send_json_error([
            'message' => __('Invalid parameters', 'yoursite'),
            'code' => 'invalid_parameters'
        ]);
    }
    ...
    if ($result !== false) {
        ...
        wp_send_json_error([
            'message' => __('Failed to update currency status', 'yoursite'),
            'code' => 'update_failed'
        ]);
    }
}
...
function yoursite_ajax_update_conversion_rate() {
    ...
    // Get old rate for history logging
    ...
    if ($result !== false) {
        ...
        wp_send_json_error([
            'message' => __('Failed to update conversion rate', 'yoursite'),
            'code' => 'update_failed'
        ]);
    }
}
